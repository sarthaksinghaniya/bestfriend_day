...
export default function BestFriendLetter() {
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-8">
      <div className="absolute inset-0 -z-20 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.24),transparent_35%),radial-gradient(circle_at_bottom_right,rgba(233,214,255,0.32),transparent_30%)]" />
      <div className="absolute inset-0 -z-20 bg-[linear-gradient(135deg,rgba(247,242,255,0.96),rgba(238,232,247,0.94),rgba(251,244,250,0.98))] bg-[length:300%_300%] animate-[gradient-shift_18s_ease-in-out_infinite]" />

      <motion.section
        initial={{ opacity: 0, y: 18 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.7, ease: 'easeOut' }}
        className="w-full max-w-3xl rounded-[2rem] border border-white/70 bg-white/55 px-6 py-10 text-center shadow-[0_24px_80px_rgba(86,58,126,0.12)] backdrop-blur-2xl sm:px-10 sm:py-14"
      >
            ...
            {lines.slice(0, visibleCount).map((line, index) => (
              <motion.p
                ...
                className={`whitespace-pre-wrap text-lg leading-8 sm:text-xl ${
                  line ? 'mb-3' : 'mb-1'
                } ${index === 5 || index === 16 ? 'font-medium text-[#29183f]' : 'font-light'}`}
              >
                {line}
              </motion.p>
            ))}
